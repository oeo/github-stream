...
def scanner_worker(token_manager, analyzer, event_queue, verbose, worker_id):
    ...
    while True:
        ...
        if event_data is None:  # sentinel to stop
            break
        ...
        try:
            ...
            process_event(gh, event_data, analyzer, verbose, token_manager, token)
...
def process_event(gh, event_data, analyzer, verbose, token_manager, token):
                ...
                with ThreadPoolExecutor(max_workers=10) as executor:
                    ...
                    for file in commit.files:
                        ...
                        if file.get('contents_url'):
                            # use the token for this download
...
def monitor_github_events(args):
    ...
    print(f"{Colors.HEADER}--- GitHub Stream Scanner ---{Colors.ENDC}")
    ...
    # start scanner worker threads (one per healthy token)
    num_scanner_workers = min(token_manager.get_healthy_token_count(), 4)  # max 4 parallel scanners
    scanner_workers = []
                            ...
                            for commit_data in event.payload.get('commits', []):
                                ...
                                with ThreadPoolExecutor(max_workers=args.cpus * 2) as executor:
                                    ...
                                    for file in commit.files:
                                        ...
                                        if base_filename in EXCLUDED_FILES:
                                            if verbose:
                                                print(f"  -> {Colors.WARNING}Skipping excluded file: {filename}{Colors.ENDC}")
                                            continue
                                        
                                        # skip files matching excluded patterns
                                        if any(fnmatch.fnmatch(base_filename, pattern) for pattern in EXCLUDED_PATTERNS):
                                            if verbose:
                                                print(f"  -> {Colors.WARNING}Skipping by pattern: {filename}{Colors.ENDC}")
                                            continue
                                        
                                        # check if it's a dotfile or matches target extensions
                                        is_dotfile = '/' in filename and filename.split('/')[-1].startswith('.') or filename.startswith('.')
                                        
                                        # scan if it's either a dotfile or has a target extension
                                        if not (is_dotfile or any(filename.endswith(ext) for ext in TARGET_EXTENSIONS)):
                                            continue
                                        
                                        # log dotfile detection for visibility
                                        if is_dotfile and verbose:
                                            print(f"  -> {Colors.OKCYAN}Detected dotfile: {filename}{Colors.ENDC}")
                                ...
                                for file_data in files_to_process:
                                    ...
                                    if true_file_size < MIN_FILE_SIZE:
                                        if verbose:
                                            print(f"    -> {Colors.WARNING}Skipping tiny file ({true_file_size} B): {file_data['filename']}{Colors.ENDC}")
                                        continue
                                    
                                    # skip files that are too large
                                    if true_file_size > MAX_FILE_SIZE:
                                        if verbose:
                                            size_str = f"{true_file_size / 1024:.2f} KB"
                                            print(f"    -> {Colors.WARNING}Skipping large file ({size_str}): {file_data['filename']}{Colors.ENDC}")
                                        continue
